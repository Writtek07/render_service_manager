class Service < ApplicationRecord
    after_create :update_schedule_file_on_create, :create_rake_task
    ...
    private
    def update_schedule_file_on_create
        ...
        new_lines << "every :day, at: '#{custom_start_time.strftime("%I:%M")}' do\n"        
        new_lines << "  rake 'services_#{id}:start'\n"
        new_lines << "  runner \"puts '================== End of start service call for service:#{name} ================='\"\n"
        new_lines << "end\n"
        new_lines << "\n"
        new_lines << "every :day, at: '#{custom_stop_time.strftime("%I:%M")}' do\n"        
